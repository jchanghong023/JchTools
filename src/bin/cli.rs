...
fn argument(args:&[String],flag:&str)->Option<String>{args.iter().position(|s|s==flag).and_then(|i|args.get(i+1)).cloned()}
fn run()->Result<()> {
    ...
    match args.first().map(String::as_str){
        ...
        Some("analyze")=>{
            ...
            if cfg.extract_conflict==ConflictPolicy::Ask{cfg.extract_conflict=ConflictPolicy::KeepBoth;}
