...
- **产品**：JchTools——用户个人使用的 Windows 本地工具箱。技术栈 Rust 2021 + Slint 1.17（自绘界面、fluent 风格）、SQLite（rusqlite bundled）、随包 7-Zip 引擎。产品定位与命名规范见合同 P 分区。
...
| 代理工具 | `cargo test`（`src/proxy.rs` / `src/nettest.rs` 单测）；真实网络连通、外网 IP 与命令复制由用户本人点击验收，自动化不冒充 | 合同 X 分区 |
