...
- **递归解压**：选目录 → 调整规则 → 点「开始解压」→ 弹一次确认（告知发现的压缩包数量、就地解压去向、成功原包移入回收站、失败原包移入「解压失败」）→ 确认后连续执行到结束。每个压缩包解到它所在的位置；成功完全解开的原包按处置策略处理（默认回收站），未能完全解开的（损坏、加密、格式不支持、分卷不全、触防护上限、含被拒绝的危险条目）整组移入所选目录根下的「解压失败」并记录原因，重跑默认跳过该目录里的包。
...
| 原压缩包 | 保留、回收、永久删除；未能完全解开的包（损坏、加密、不支持、分卷不全、触防护上限、含被拒绝的危险条目）整组移入「解压失败」并记录原因；成功分卷组整组处置，宽命名兄弟卷（同主干 .rNN/.zNN）须引擎分卷佐证才随包处置 |
...
`src/main.rs`/`ui/app.slint`：界面；`resources/rules.json`：39 项界面规则；`src/config.rs`：配置及校验；`src/registry.rs`：真实工具注册；`src/engine.rs`：阶段控制；`archive.rs`：7-Zip；`planner.rs`：计划；`platform.rs`：回收站/删除；`db.rs`/`schema.sql`：磁盘索引、计划及审计；`tests/`：测试；`scripts/`：检查/打包；`.github/workflows/check.yml`：Windows CI。
