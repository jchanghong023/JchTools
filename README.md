...
导航只显示已注册工具（递归解压、目录整理）和关于；工具描述集中注册，增加新工具时才出现其真实入口，当前未包含任何空 AI/文档分类。「关于」包含 Slint 的 AboutSlint 署名组件。不提供独立的任务记录页或设置页：规则只在主界面「处理规则」里会话内调整，不落盘、不导入导出。软件自身不发起网络请求；用户明确点击官方链接时可以调用系统浏览器。
...
| 归类 | 固定「大类/功能分类」两级结构；大类按扩展名映射（附录 A），功能分类由文件名与紧邻原父目录名经确定性统计聚类生成（纯本地、不读正文、不用模型），每个大类至多 10 个功能目录，无法可靠分类的进入「其他」；可单独归类超大文件 |
...
两个工具分别测试，不会自动串联：递归解压确认原包/分卷保留、冲突另存、嵌套产物落盘及失败隔离；重新生成干净数据集后，再测试目录整理的分析、计划勾选、执行和空目录收尾。隐藏/系统属性文件默认包含，缩小范围时应出现提示。
