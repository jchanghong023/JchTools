...
导航只显示已注册工具（目录整理、代理工具）和关于；工具描述集中注册，增加新工具时才出现其真实入口。当前未包含任何空 AI/文档分类。“关于”包含 Slint 的 AboutSlint 署名组件。不提供独立的任务记录页或设置页：规则只在主界面「处理规则」里会话内调整，不落盘、不导入导出。代理工具只读检测环境变量、系统代理、常见 VPN/代理进程与虚拟网卡，并提供可复制的设置命令，不自动修改系统。
...
- 两组切换用两种样式：面板切换（处理规则 / 整理计划 / 进度与日志）是下划线页签，规则分区（解压 / 去重 / 冲突 / 归类 / 清理 / 安全与性能 / 应用）是实心胶囊，不会混淆。
